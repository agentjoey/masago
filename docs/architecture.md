...
6. 重试复用已持久化输出，不重复调用 LLM；

   **例外与理由（2026-08-12 实现时发现的规则冲突）：** 本条原文要求"不重复调用 LLM/TTS"，但在"TTS 合成成功、Telegram 发送失败"这一窄窗口内，它与 §10.3"生成音频处理后删除"直接冲突——要复用就得把音频落库。

   裁决为**保留 §10.3，接受重试时重新合成**。理由是量级：一次重合成约 150 日文字符，按 Turbo $60/1M 计约 **$0.000009**，而为满足条文把生成音频持久化会实质性削弱数据最小化，代价远大于收益。本条真正要防的是重复调用 LLM（每次约 $0.01–0.03，比 TTS 高三个数量级），那部分已由 §8.2 的状态机保证并有测试覆盖。

   若未来 TTS 单价或重试频率显著上升，再以 outbox 持久化音频引用的方式重新评估。
...
四条合起来，数据库的活跃时间约等于"用户实际学习时间 + 每次活动后的休眠延迟"。

**实测值（2026-08-12，jp-coach 项目 `old-tree-65966377`）：** 最后一次查询 09:46:15，compute 于 09:54:47 进入 `idle`，**实际休眠延迟 8 分 32 秒**（5 分钟阈值 + Neon 侧调度滞后）。据此重算典型一天：25 分钟 session + 1 次提醒 + 两段 ~8.5 分钟延迟 ≈ **45 分钟/天 ≈ 22.5 小时/月 ≈ 5.6 CU-hours/月**，相对 100 的额度约有 **18 倍余量**。

此测量是在应用未运行、仅有测试流量的条件下取得的，确认了「代码中无保活逻辑 → compute 确实会 suspend」这一因果链。真实负载下的确认仍需 §14 阶段 5 的连续 7 天观测。
