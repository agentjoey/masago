...
>
> **状态：方案 A 已实现并上线**（2026-08-14）。方案 B（OAuth）仍是设计。
>
> 线上端点：`https://masago-production.up.railway.app/mcp/<token>`
...
六个工具全部上线，用 SDK 的真实客户端从握手到调用跑通了（`tests/mcp/`）。
线上实测：`fetch("kana:a")` 返回学习者真实的 FSRS 状态（练习 2 次、
下次复习 2026-08-21、REVIEW 阶段）。

**上线时撞到两个问题，都不在设计里：**

**① 生产环境跑的是 Node 18。** MCP 的 transport 用全局 Web Crypto（Node 19+
才有），第一次调用就 `ReferenceError: crypto is not defined`。真正的问题不是
crypto，是三个 Node 版本互不相同：本地 25 / CI 22 / 生产 18。**决定合否的是
CI，所以让生产对齐 CI**——`engines.node: "22.x"`，并在启动时也检查一次
（与其让第一个用户撞上，不如起不来）。启动日志现在会记 Node 版本，
这次就是因为日志里没有才绕了远路。

**② 两处数字对不上——而且是我自己造的。** 设计里 §5 的风险表专门列了这条，
结果周报用 `VOCAB.length`（1374），Mini App 用 `vocabProgress()`（1301）。
差的 73 个是接尾辞（`～円`『～分』），单独不教，不进分母。已收敛到一个函数，
并加了测试：这两个文件里不许出现 `VOCAB.length`。
