...
describe('kana command routing', () => {
  function setupKana() {
    const kanaCommands = {
      ...
      write: vi.fn().mockResolvedValue([{ text: 'R_WRITE' }]),
      read: vi.fn().mockResolvedValue([{ text: 'R_READ' }]),
      compose: vi.fn().mockResolvedValue([{ text: 'R_COMPOSE' }]),
      domain: vi.fn().mockResolvedValue([{ text: 'R_DOMAIN' }]),
  ...
  it('lists only commands that are actually registered', async () => {
    const { bot, apiCalls } = (() => {
      const kanaCommands = {
        ...
        write: vi.fn().mockResolvedValue([{ text: 'x' }]),
        read: vi.fn().mockResolvedValue([{ text: 'x' }]),
        compose: vi.fn().mockResolvedValue([{ text: 'x' }]),
        domain: vi.fn().mockResolvedValue([{ text: 'x' }]),
